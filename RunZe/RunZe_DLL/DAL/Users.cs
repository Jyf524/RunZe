...
namespace Maticsoft.DAL
{
	...
	public partial class Users
	{
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select UserID,Username,UserPassword,UserRealName,UserSex,UserEmail,UserGrade,UserScore,Province,City,Address1,UserIdentity,RegistTime ");
            strSql.Append(" FROM Users ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere + " order by RegistTime desc ");
            }
            if (strWhere.Trim() == "")
            {
                strSql.Append("order by RegistTime desc ");
            }
            return DbHelperSQL.Query(strSql.ToString());
        }
