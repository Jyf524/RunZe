...
namespace Maticsoft.DAL
{
    ...
    public partial class Appraise
    {
        ...
        public bool Exists(string AppriseID)
        {
            ...
            strSql.Append(" where AppriseID='" + AppriseID + "' ");
        ...
        public bool Add(Maticsoft.Model.Appraise model)
        {
            ...
            if (model.AppriseID != null)
            {
                strSql1.Append("AppriseID,");
                strSql2.Append("'" + model.AppriseID + "',");
            }
        ...
        public bool Update(Maticsoft.Model.Appraise model)
        {
            ...
            strSql.Remove(n, 1);
            strSql.Append(" where AppriseID='" + model.AppriseID + "' ");
        ...
        public bool DeleteList(string AppriseIDlist)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from Appraise ");
            strSql.Append(" where AppriseID in (" + AppriseIDlist + ")  ");
            int rows = DbHelperSQL.ExecuteSql(strSql.ToString());
            if (rows > 0)
            {
                return true;
        ...
        public bool DeleteList2(string AppriseIDlist)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from Appraise ");
            strSql.Append(" where UserID in (" + AppriseIDlist + ")  ");
            int rows = DbHelperSQL.ExecuteSql(strSql.ToString());
            if (rows > 0)
            {
                return true;
        ...
        public Maticsoft.Model.Appraise GetModel(string AppriseID)
        {
            ...
            strSql.Append(" AppriseID,UserID,CommodityID ");
        ...
        /// 得到一个对象实体
        /// </summary>
        public Maticsoft.Model.Appraise DataRowToModel(DataRow row)
        {
            Maticsoft.Model.Appraise model = new Maticsoft.Model.Appraise();
            if (row != null)
            {
                if (row["AppriseID"] != null)
                {
                    model.AppriseID = row["AppriseID"].ToString();
                }
                if (row["UserID"] != null)
                {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select AppriseID,UserID,CommodityID ");
            strSql.Append(" FROM Appraise ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            {
                strSql.Append("order by T.AppriseID desc");
            }
