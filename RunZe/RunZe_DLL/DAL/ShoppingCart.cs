...
namespace Maticsoft.DAL
{
    ...
    public partial class ShoppingCart
    {
        ...
        public bool DeleteList2(string ShoppingCartIDlist)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from ShoppingCart ");
            strSql.Append(" where UserID in (" + ShoppingCartIDlist + ")  ");
            int rows = DbHelperSQL.ExecuteSql(strSql.ToString());
            if (rows > 0)
            {
                return true;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select distinct ShoppingCartID,a.UserID,a.CommodityID,OrderNumber,Subtotal,b.CommodityID,CommodityFatherID,CommoditySonID,CommodityName,CommodityPrice,MarketPrice,VIPPrice,Stock,BuyScore,CommodityImage,CommodityType,CommoditySaled,Introduce,CommodityTime,Recommend,CommodityState ");
            strSql.Append(" FROM ShoppingCart a,Commodity b  ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) FROM ShoppingCart a,Commodity b ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) FROM OrderDetail a,Commodity b ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
